...
class MovieModel {
  ...
  var apiKey = "925e10a596561f3cfbd009f1a71151b5";
  ...
  Future<List<MovieCard>> getMovies({
    ...
    var data = await _getData(
      url: '$kThemoviedbURL/$mTypString?api_key=$apiKey&language=ta-IN&page=1',
    );
    print("tamil movies $data");
  ...
  Future<List<MovieCard>> searchMovies({
    ...
    var data = await _getData(
      url:
          '$kThemoviedbSearchURL/?api_key=$apiKey&language=ta-IN&page=1&include_adult=false&query=$movieName',
    );
  ...
  Future<MovieDetails> getMovieDetails({required String movieID}) async {
    var data = await _getData(
      url:
          '$kThemoviedbURL/$movieID?api_key=$apiKey&language=en-US',
    );
  ...
  Future<List<MovieCard>> getFavorites(
    ...
    for (var item in favoritesID) {
      if (item != "") {
        var data = await _getData(
          url:
              '$kThemoviedbURL/$item?api_key=$apiKey&language=en-US',
        );
